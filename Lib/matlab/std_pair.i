...
%fragment("StdPairTraits","header",fragment="StdTraits") {
  namespace swig {
    ...
    struct traits_asval<std::pair<T,U> >  {
      ...
      static int get_pair(const matlab_value& first, matlab_value second,
			  std::pair<T,U> *val)
      {
	if (val) {
	  T *pfirst = &(val->first);
      ...
      static int asval(const matlab_value& obj, std::pair<T,U> *val) {
	if (obj.is_cell()) {
	...
	return SWIG_ERROR;
      ...
      static int asptr(const matlab_value& obj, std::pair<T,U> **val) {
	if (obj.is_cell()) {
	  Cell c=obj.cell_value();
	  if (c.numel()<2) {
	    error("pair from Cell array requires at least two elements");
    ...
    struct traits_from<std::pair<T,U> >   {
      static matlab_value from(const std::pair<T,U>& val) {
	Cell c(1,2);
