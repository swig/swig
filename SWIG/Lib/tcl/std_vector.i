...
namespace std {
	...
	if(SWIG_ConvertPtr(interp, $input, (void **) &v, \
	  ...
	  for (i = 0; i < nitems; i++) {
		...
		char message[] = "list of type $descriptor(T) expected";
	...
	for (unsigned int i=0; i<$1.size(); i++) {
	  ...
	  Tcl_ListObjAppendElement(interp, $result, \
				   ...
				   SWIG_NewPointerObj(ptr, $descriptor(T), 0));
	...
	if(SWIG_ConvertPtr(interp, $input, (void **) &v, \
	  ...
	  if(Tcl_ListObjGetElements(interp, $input, &nitems, &listobjv) == TCL_ERROR)
	    ...
	    else if ((SWIG_ConvertPtr(interp, listobjv[i],(void **) &temp,
    ...
    specialize_std_vector(unsigned int, SwigInt_As<unsigned int>, Tcl_NewIntObj);

    specialize_std_vector(unsigned short, SwigInt_As<unsigned short>, Tcl_NewIntObj);

    specialize_std_vector(unsigned long, SwigInt_As<unsigned long>, Tcl_NewIntObj);
    ...
    specialize_std_vector(std::string, SwigString_AsString, SwigString_FromString);
