...
MODULA3 ():
  ...
  String *getMappedTypeNew (Node * n, const char *map, const char *lname =
                            "", bool warn = true) {
...
//printf("dir for %s: %s\n", Char(Getattr(p,"name")), Char(dir));
   ...
   * ----------------------------------------------------------------------------- */
  String *prefixedNameToModula3 (Node * n, const String * sym,
                                 bool leadingCap)
  {
    ...
    String *suffix = nameToModula3 (short_sym, leadingCap
                                    || hasContent (newPrefix));
...
//printf("evaluation: return %ld\n", numvalue);
  ...
  struct writeArgState
  {
    ...
    bool hold;
      writeArgState ():mode (NIL), name (NIL), type (NIL), value (NIL),
      hold (false)
    {
    };
  };

  void writeArg (File * f, writeArgState & state,
                 String * mode, String * name, String * type, String * value)
  {
    ...
    if (state.name != NIL) {
      ...
      if ((name != NIL) &&
          equalNilStr (state.mode, mode) &&
          equalNilStr (state.type, type) &&
          (state.value == NIL) && (value == NIL)
  ...
  virtual int top (Node * n)
  {
    if (hasContent (constantfilename) ||
        hasContent (renamefilename) || hasContent (typemapfilename)) {
      int result = SWIG_OK;
      if (hasContent (constantfilename)) {
...
\n", input_file);
  ...
  void scanTypemap (File * file, Node * n)
  {
    ...
    while (child != NIL) {
      ...
      if ((Strcmp (type, "class") == 0) ||
          ((Strcmp (type, "cdecl") == 0) && (storage != NIL)
...
\n", input_file);
  ...
  int generateM3Top (Node * n)
  {
    ...
    {
      File *file =
        openWriteFile (NewStringf
                       ("%sm3makefile", Swig_file_dirname (outfile)));
    ...
    // Generate the raw interface
    {
      File *file =
        openWriteFile (NewStringf
                       ("%s%s.i3", Swig_file_dirname (outfile), m3raw_name));
    ...
    // Generate the raw module
    {
      File *file =
        openWriteFile (NewStringf
                       ("%s%s.m3", Swig_file_dirname (outfile), m3raw_name));
    ...
    // Generate the interface for the comfort wrappers
    {
      File *file =
        openWriteFile (NewStringf
                       ("%s%s.i3", Swig_file_dirname (outfile), m3wrap_name));
    ...
    // Generate the wrapper routines implemented in Modula 3
    {
      File *file =
        openWriteFile (NewStringf
                       ("%s%s.m3", Swig_file_dirname (outfile), m3wrap_name));
    ...
    for (Iterator swig_type = First (swig_types_hash); swig_type.item != NIL;
         swig_type = Next (swig_type)) {
...
// a Java relict
  ...
  virtual int emitM3RawPrototype (Node * n, const String * cname,
                                  const String * m3name)
  {
    ...
    // Generate signature for raw interface
    {
      ...
      for (p = skipIgnored (l, "m3rawintype"); p != NULL;
           p = skipIgnored (p, "m3rawintype")) {
  ...
  void generateIntConstant (Node * n, String * name)
  {
    ...
    String *m3value;
    if ((conv == NIL) ||
        ((Strcmp (conv, "set:int") != 0) &&
         (Strcmp (conv, "int:set") != 0))) {
    ...
    while (numvalue > 0) {
      if ((numvalue & 1) != 0) {
        if (isIntType) {
          ...
          if (bitname == NIL) {
            Swig_warning (WARN_MODULA3_BAD_ENUMERATION, input_file,
                          line_number,
  ...
  void generateConstant (Node * n)
  {
    ...
    if (hasContent (enumitem) ||
        hasContent (constset) || hasContent (constint)) {
...
#if 0
  void generateEnumerationItem (const String * name, const String * value,
                                int numvalue)
  {
...
*/
  ...
  void Setfeature (Node * n, const char *feature, const String * value,
                   bool warn = false) {
  ...
  {
    if ((mode == NIL) ||
        (Strcmp (mode, "int:int") == 0) || (Strcmp (mode, "set:set") == 0)) {
      out = in;
      return true;
...
//printf("items %lx, set %s = %s\n", (long) items, Char(newvalue), Char(m3name));
  ...
  enum const_pragma_type
  { cpt_none, cpt_constint, cpt_constset, cpt_enumitem };
  ...
  void tagConstants (Node * first, String * parentEnum,
                     const const_id_pattern & pat, const String * pragma,
                     List * convdesc)
  {
    Node *n = first;
    while (n != NIL) {
      ...
      if ((isConstant || isEnumItem) &&
        ...
        if (Strcmp (srcstyle, "underscore") == 0) {
          ...
          if (srcstyle != NIL) {
            Swig_warning (WARN_MODULA3_BAD_ENUMERATION, input_file,
                          line_number, "Unknown C identifier style <%s>.\n",
                          srcstyle);
          }
          newname = Copy (name);
        }
        ...
        if (Strcmp (pragma, "enumitem") == 0) {
          if (Len (relationdesc) != 1) {
            Swig_warning (WARN_MODULA3_BAD_ENUMERATION, input_file,
                          line_number, "Expected <enumeration>, got <%s>.\n",
                          relationstr);
          }
          Setfeature (n, "modula3:enumitem:name", newname, true);
          Setfeature (n, "modula3:enumitem:enum", relationstr, true);
          Setfeature (n, "modula3:enumitem:conv",
                      NewStringf ("%s:int", srctype), true);
        } else if (Strcmp (pragma, "constint") == 0) {
          if (Len (relationdesc) != 1) {
            Swig_warning (WARN_MODULA3_BAD_ENUMERATION, input_file,
                          line_number, "Expected <ordinal type>, got <%s>.\n",
                          relationstr);
          }
          Setfeature (n, "modula3:constint:name", newname, true);
          Setfeature (n, "modula3:constint:type", Getitem (relationdesc, 0),
                      true);
          Setfeature (n, "modula3:constint:conv",
                      NewStringf ("%s:int", srctype), true);
          ...
          Setfeature (n, "modula3:constset:base", Getitem (relationdesc, 1),
                      true);
          Setfeature (n, "modula3:constset:conv",
                      NewStringf ("%s:set", srctype), true);
        }
  ...
  void emitProxyClassDefAndCPPCasts (Node * n)
  {
    ...
    const String *pure_baseclass =
      typemapLookup ("m3base", classDeclarationName, WARN_NONE);
    ...
    const String *pure_interfaces =
      typemapLookup (derived ? "m3interfaces_derived" : "m3interfaces",
    ...
    if (derived) {
      tm =
        typemapLookup ("m3destruct_derived", classDeclarationName, WARN_NONE,
                       attributes);
      destruct_methodname =
        Getattr (attributes, "tmap:m3destruct_derived:methodname");
    } else {
      tm =
        typemapLookup ("m3destruct", classDeclarationName, WARN_NONE,
                       attributes);
      destruct_methodname =
        Getattr (attributes, "tmap:m3destruct:methodname");
    }
    ...
    if (tm) {
      ...
      if (*Char (destructor_call)) {
        Printv (proxy_class_def,
                typemapLookup ("m3finalize", classDeclarationName, WARN_NONE),
                NIL);
      }
      // Dispose method
      ...
      if (*Char (destruct))
        Printv (proxy_class_def, "\n  public ",
                derived ? "override" : "virtual", " void ",
                destruct_methodname, "() ", destruct, "\n", NIL);
    }
    Delete (attributes);
    Delete (destruct);
    ...
    if (derived) {
      Printv (m3raw_cppcasts_code, "\n  [DllImport(\"", m3wrap_name,
              "\", EntryPoint=\"Modula3_", proxy_class_name, "To", baseclass,
              "\")]\n", NIL);
      Printv (m3raw_cppcasts_code, "  public static extern IntPtr ",
  ...
  virtual int classHandler (Node * n)
  {
...

    {
      ...
      if (Cmp (kind, "struct") == 0) {
        ...
        Node *child;
        writeArgState state;
        for (child = firstChild (n); child != NIL;
             child = nextSibling (child)) {
...

        {
          ...
          for (acc = acc_public; acc <= acc_private; acc++) {
            ...
            if (hasContent (methods[acc]) || process_private) {
              String *subclass =
                NewStringf ("%s%s", proxy_class_name, acc_m3suffix[acc]);
              /*
        ...
        Swig_warning (WARN_MODULA3_TYPECONSTRUCTOR_UNKNOWN, input_file,
                      line_number, "Unknown type constructor %s\n", kind);
      }
    }
...
*/
      ...
      if ((!isVirtual) || (!isOverridden)) {
        ...
        }
        {
          ...
          String *overrides =
            getAttrString (parentNode (n), "modula3:override");
  ...
  void proxyClassFunctionHandler (Node * n)
  {
    ...
    if (proxy_flag && wrapping_member_flag && !enum_constant_flag) {
      ...
      setter_flag =
        (Cmp
         (Getattr (n, "sym:name"),
          Swig_name_set (Swig_name_member (proxy_class_name, variable_name)))
         == 0);
    }
    ...
    for (i = 0, p = l; p; i++) {
      ...
      if (!(variable_wrapper_flag && i == 0)) {
        ...
        String *arg =
          variable_wrapper_flag ? NewString ("value") : makeParameterName (n,
                                                                           p,
                                                                           i);
  ...
  virtual int constructorHandler (Node * n)
  {
    ...
    if (proxy_flag) {
      ...
      Printf (proxy_class_code, "  %s %s(",
              Getattr (n, "feature:modula3:methodmodifiers"),
              proxy_class_name);
      Printv (imcall, " : this(", m3raw_name, ".",
              Swig_name_construct (overloaded_name), "(", NIL);
  ...
  virtual int destructorHandler (Node * n)
  {
    ...
    if (proxy_flag) {
      Printv (destructor_call, m3raw_name, ".", Swig_name_destroy (symname),
              "(swigCPtr)", NIL);
    }
    return SWIG_OK;
  }
  ...
  virtual int staticmembervariableHandler (Node * n)
  {
    ...
    if (static_const_member_flag) {
      ...
      // Output the property's field declaration and accessor methods
      Printf (proxy_class_code, "  public static %s %s {", tm,
              Getattr (n, "sym:name"));
    }
...
*/
    ...
    /* Get return types */
    String *result_m3rawtype =
      Copy (getMappedTypeNew (n, "m3rawrettype", ""));
    String *result_m3wraptype =
      Copy (getMappedTypeNew (n, "m3wraprettype", ""));
       ...
       and determine return type that might result from a converted VAR argument. */
    {
      ...
      if (multiretval && has_return_m3) {
        writeArg (return_variables, state, NIL, NewString (result_return),
                  result_m3wraptype, NIL);
      }
      ...
      while (p != NIL) {
        ...
        if (tm != NIL) {
          ...
          p =
            skipIgnored (Getattr (p, "tmap:m3wrapouttype:next"),
                         "m3wrapouttype");
        } else {
          p = nextSibling (p);
        }
...

    {
      ...
      if (!has_return_raw) {
        ...
        String *return_conv =
          getMappedTypeNew (n, "m3wrapretconv", "", false);
  ...
  void attachParameterNames (Node * n, const char *tmid, const char *nameid,
                             const char *fmt)
  {
    ...
    while (p != NIL) {
      ...
      if (1 == Setattr (hash, newname, "1")) {
        Swig_warning (WARN_MODULA3_DOUBLE_ID, input_file, line_number,
                      "Argument '%s' twice.\n", newname);
      }
  ...
  String *createM3Signature (Node * n)
  {
    ...
    while (p != NIL) {
      ...
      if (tm != NIL) {
        ...
        p =
          skipIgnored (Getattr (p, "tmap:m3wrapintype:next"), "m3wrapintype");
      } else {
        p = nextSibling (p);
      }
    }
...
/* not used any longer
  ...
  void emitTypeWrapperClass (String * classname, SwigType * type)
  {
    ...
    const String *pure_interfaces =
      typemapLookup ("m3interfaces", type, WARN_NONE);
  ...
  int addThrows (Hash * throws_hash, const String * typemap, Node * parameter)
  {
    ...
    String *throws =
      getMappedTypeNew (parameter, Char (throws_attribute), "", false);
    ...
    if (throws) {
      ...
      if (temp_classes_list /*&& hasContent(temp_classes_list) */ ) {
        for (Iterator cls = First (temp_classes_list); cls.item != NIL;
             cls = Next (cls)) {
  ...
  void addImports (Hash * imports_hash, const String * typemap, Node * node)
  {
    ...
    String *imports =
      getMappedTypeNew (node, Char (imports_attribute), "", false);
    ...
    if (imports != NIL) {
      ...
      if (import_list != NIL) {
        for (Iterator imp = First (import_list); imp.item != NIL;
             imp = Next (imp)) {
