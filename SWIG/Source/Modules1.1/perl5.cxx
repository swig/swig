...
//
// module
//   set by the %module directive, it will set package if the -package 
//   flag is not used. It will determine the name of the .pm file
//
...
//   package with a 'c' appended
//
static String       *cmodule = 0; 
//
// package 
//   the initial value of package, without any namespace
  ...
  for (i = 1; i < argc; i++) {
	  ...
	  if(strcmp(argv[i],"-package") == 0) {
	    if (argv[i+1]) {
	      ...
	      Swig_mark_arg(i+1);
	      i++;
	      Printf(stderr,"*** -package is depricated use -namesapce instead'\n");
	    } else {
	      Swig_arg_error();
	    }
            ...
            if (argv[i+1]) {
              ...
              Swig_mark_arg(i+1);
              i++;
	      Printf(stderr,"*** -interface is depricated use -namesapce instead'\n");
            } else {
              Swig_arg_error();
            }
...
PERL5::top(Node *n) {
  ...
  if (blessed) {
    cmodule = pkg_namespace ? NewStringf("%s::%sc", pkg_namespace,package) 
                        : NewStringf("%sc",package);
  } else {
    cmodule = package;
  }
  ...
  {
    String *tmp = NewString(package);
    //String *tmp = NewString(smodule);
    Replaceall(tmp,":","_");
...
PERL5::classHandler(Node *n) {
  ...
  if (blessed) {
    ...
    if (!compat) {
      fullclassname = NewStringf("%s::%s",package,class_name);
    } else {
