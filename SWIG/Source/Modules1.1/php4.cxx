...
void
PHP4::parse_args(int argc, char *argv[]) {
	int i;
	strcpy(LibDir, "php4");
	...
	for(i = 1; i < argc; i++) {
	  if(argv[i]) {
	    ...
	    } else if(strcmp(argv[i], "-noextra") == 0) {
		    gen_extra = 0;
		    Swig_mark_arg(i);
	    }
...
static const char *swig_header = "/*\
 --------------------------------------------------------------------------\
\n * This file was automatically generated by SWIG (http://www.swig.org).\
\n * Version 1.3.6u-20010926-1417\
\n *\
...
PHP4::initialize(void) {
  ...
  sprintf(filen,"%sphp_%s.h", output_dir,Char(module));
  if((f_h = fopen(filen, "w")) == 0) {
	Printf(stderr,"Unable to open %s\n", filen);
	SWIG_exit(EXIT_FAILURE);
  }

  Printf(f_h, swig_header);
...
void
PHP4::create_function(char *name, char *iname, SwigType *d, ParmList *l) {
  ...
  sprintf(args, "%s[%d]", "zval **args", pcount); 
    ...
    if(!Getignore(p)) {
	  /* Check if optional */
	    ...
	    temp[0] = 0;
    	  ...
    	  /* Insert typemap check code if exists */
	  ...
	  if((tm = Swig_typemap_lookup((char*)"argout",pt,pn,target,0,0))) {
	 ...
	 if((tm = Swig_typemap_lookup((char*)"out",d,iname,(char*)"result",(char*)"return_value",0))) {
		Printf(f->code, "%s\n", tm);
	 } else {
...
void
PHP4::link_variable(char *name, char *iname, SwigType *t)
{
  int flags = 0;
...
void
PHP4::declare_const(char *name, char *iname, SwigType *type, char *value)
{
