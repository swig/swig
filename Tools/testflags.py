...
"""
Display CFLAGS or CXXFLAGS to use for testing the SWIG examples
and test-suite.
"""
...
def get_cflags(language, std, compiler):
    """Get cflags."""
    if std is None or not std:
        std = "gnu89"
    c_common = "-fdiagnostics-show-option -std=" + std + \
        " -Wno-long-long -Wreturn-type -Wdeclaration-after-statement" \
...
def get_cxxflags(language, std, compiler):
    ...
    if std is None or not std:
        std = "c++98"
    cxx_common = "-fdiagnostics-show-option -std=" + std + \
        " -Wno-long-long -Wreturn-type -Wmissing-field-initializers"
    cxxflags = {
        ...
        # Until overload_rename is fixed for node
        "javascript": "-Werror " + cxx_common + " -Wno-error=unused-function",
        ...
        "r": "-Werror " + cxx_common,
        # For Ruby on MacOS Xcode 9.4 misconfiguration defining 'isfinite'
        # to deprecated 'finite'
        "ruby": "-Werror " + cxx_common + " -Wno-deprecated-declarations",
        "scilab": "-Werror " + cxx_common,
        "tcl": "-Werror " + cxx_common,
    }
    if compiler == "clang":
        # -Wno-attributes is for clang LLVM 3.5 and bdw-gc < 7.5 used by guile
        cxxflags["guile"] += " -Wno-attributes"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Display CFLAGS or CXXFLAGS to use for testing the SWIG "
        "examples and test-suite.")
    parser.add_argument("-l", "--language", required=True,
                        help="set language to show flags for")
    flags = parser.add_mutually_exclusive_group(required=True)
    flags.add_argument("-c", "--cflags", action="store_true",
                       default=False, help="show CFLAGS")
    flags.add_argument("-x", "--cxxflags", action="store_true",
                       default=False, help="show CXXFLAGS")
    parser.add_argument("-s", "--std", required=False,
                        help="language standard flags for the -std= option")
    parser.add_argument("-C", "--compiler", required=False,
                        help="compiler used (clang or gcc)")
