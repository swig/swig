...
def filterFiles(all_files):
    ...
    for f in all_files:
        ...
        if f == ymlfile:
            ...
            # Other YMAL files
            skip=True
        elif re.match('^Examples/test-suite/errors', f):
            m = 'ccache'
        elif re.match('^Examples/test-suite/schemerunme', f):
            m = 'guile'
        elif re.match(r'^Examples/test-suite/.*_.*\.i$', f):
            m = re.sub(r'^Examples/test-suite/([^_]+)_.*\.i$', r'\1', f)
        elif re.match('^Examples/test-suite/', f):
            m = re.sub('^Examples/test-suite/([^/]+)/.*', r'\1',f)
        elif re.match('^Examples/', f):
            m = re.sub('^Examples/([^/]+)/.*', r'\1',f)
        elif re.match('^Lib/', f):
            m = re.sub('^Lib/([^/]+)/.*', r'\1',f)
        elif re.match(r'^Source/Modules/.*\.cxx$', f):
            m = re.sub(r'^Source/Modules/([^.]+)\.cxx$', r'\1',f)
        elif re.match('^Tools/javascript/', f):
            m = 'javascript'
        elif re.match('^CCache/', f):
