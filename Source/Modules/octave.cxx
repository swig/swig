...
public:
  ...
  void process_autodoc(Node* n) {
    ...
    if (!d) {
      d = NewHash();
      Setattr(d, "synopsis", NewStringEmpty());
      Setattr(d, "decl_info", NewStringEmpty());
      Setattr(d, "cdecl_info", NewStringEmpty());
      Setattr(d, "args_info", NewStringEmpty());
    ...
    if (autodoc_enabled) {
      String* decl_str = NewStringEmpty();
      String* args_str = NewStringEmpty();
  ...
  void make_autodocParmList(Node* n, String* decl_str, String* args_str) {
    ...
    for (p = plist; p; p = pnext) {
      ...
      String* tex_name = NewStringEmpty();
      if (name) {
  ...
  virtual int functionWrapper(Node* n) {
    ...
    Octave_begin_function(n, w->def, iname, overname, !overloaded);

    emit_parameter_variables(l, w);
    ...
    if (constructor && num_arguments == 1 && num_required == 1) {
      if (Cmp(storage, "explicit") == 0) {
        ...
        if (GetFlag(parent, "feature:implicitconv")) {
          ...
          Printf(w->code, "if (SWIG_CheckImplicit(%s)) SWIG_fail;\n", desc);
          Delete(desc);
        }
      }
    }
    ...
    if (director_method) {
      ...
      Append(w->code, "upcall = !!dynamic_cast<Swig::Director*>(arg1);\n");
    }

    Setattr(n, "wrap:name", overname);

    Swig_director_emit_dynamic_cast(n, w);
    String* actioncode = emit_action(n);

    Wrapper_add_local(w, "_out", "octave_value_list _out");
    Wrapper_add_local(w, "_outp", "octave_value_list *_outp=&_out");
    Wrapper_add_local(w, "_outv", "octave_value _outv");

    // Return the function value
    if ((tm = Swig_typemap_lookup_out("out", n, Swig_cresult_name(), w, actioncode))) {
      ...
      Printf(w->code, "%s\n", tm);
      Printf(w->code, "if (_outv.is_defined()) _outp = " "SWIG_Octave_AppendOutput(_outp, _outv);\n");
      Delete(tm);
    } else {
      Swig_warning(WARN_TYPEMAP_OUT_UNDEF, input_file, line_number, "Unable to use return type %s in function %s.\n", SwigType_str(d, 0), iname);
    }
    emit_return_variable(n, d, w);

    Printv(w->code, outarg, NIL);
    Printv(w->code, cleanup, NIL);
    ...
    Printv(w->code, cleanup, NIL);
    Printf(w->code, "return octave_value_list();\n");
    Printf(w->code, "}\n");

    /* Substitute the cleanup code */
    Replaceall(w->code, "$cleanup", cleanup);

    Replaceall(w->code, "$symname", iname);
  ...
  void dispatchFunction(Node* n) {
    ...
    Printv(w->code, dispatch, "\n", NIL);
    Printf(w->code, "error(\"No matching function for overload\");\n", iname);
    Printf(w->code, "return octave_value_list();\n");
    Printv(w->code, "}\n", NIL);
  ...
  virtual int variableWrapper(Node* n) {
    ...
    String* tm;
    Wrapper* getw = NewWrapper();
    Wrapper* setw = NewWrapper();
    ...
    if (is_assignable(n)) {
      ...
      if ((tm = Swig_typemap_lookup("varin", n, name, 0))) {
        ...
        emit_action_code(n, setw->code, tm);
        Delete(tm);
      } else {
    ...
    int addfail = 0;
    Octave_begin_function(n, getw->def, getname, getwname, true);
    Wrapper_add_local(getw, "obj", "octave_value obj");
    if ((tm = Swig_typemap_lookup("varout", n, name, 0))) {
      Replaceall(tm, "$source", name);
      Replaceall(tm, "$target", "obj");
      Replaceall(tm, "$result", "obj");
      addfail = emit_action_code(n, getw->code, tm);
      Delete(tm);
    } else {
  ...
  virtual int classHandler(Node* n) {
    ...
    String* base_class_names = NewStringEmpty();
    String* base_class = NewStringEmpty();
