...
void MATLAB::initConstant() {
  ...
  Printf(f_constants,"    mexErrMsgTxt(\"This function should only be called from inside the .m files generated by SWIG. First input should be the constant ID .\");\n");
  ...
  // Begin the switch:
  Printf(f_constants,"  int name_ok = 0;\n");
...
void MATLAB::finalizeConstant() {
  Printf(f_constants,"  default: mexErrMsgIdAndTxt(\"SWIG:RuntimeError\",\"No constant %%s.\",cmd);\n");
  Printf(f_constants,"  }\n");
  Printf(f_constants,"  if (!name_ok) {\n");
  Printf(f_constants,"    mexErrMsgIdAndTxt(\"SWIG:RuntimeError\",\"Mismatching name (%%s) for constant %%d.\",cmd,con_id);\n");
  Printf(f_constants,"  }\n");
  Printf(f_constants,"}\n");
}
