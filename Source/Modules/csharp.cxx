    ...
    for (int i = 1; i < argc; i++) {
      if (argv[i]) {
        if (strcmp(argv[i], "-dllimport") == 0) {
          ...
          Swig_mark_arg(i);
        } else if (strcmp(argv[i], "-monoaot") == 0) {
          Swig_mark_arg(i);
      ...
      for (i = first_class_dmethod; i < curr_class_dmethod; ++i) {
        ...
        if (mono_aot_compatibility_flag) {
          Printf(proxy_class_code,
                 "      swigDelegate%sdispatcher = new SwigDelegate%s_%s_Dispatcher(SwigDirector%s_Dispatcher);\n",
                 methid,
                 proxy_class_name,
                 methid,
                 overname);
          Printf(proxy_class_code,
                 "      global::System.Runtime.InteropServices.GCHandle swigDelegate%sgcHandle = "
                 "global::System.Runtime.InteropServices.GCHandle.Alloc(swigDelegate%s, global::System.Runtime.InteropServices.GCHandleType.Weak);\n",
                 methid,
                 methid);
          Printf(proxy_class_code,
                 "      swigDelegate%sgcHandlePtr = global::System.Runtime.InteropServices.GCHandle.ToIntPtr(swigDelegate%sgcHandle);\n",
                 methid,
                 methid);
        }
  ...
  int classDirectorMethod(Node *n, Node *parent, String *super) {
    ...
    if ((tm = Swig_typemap_lookup("csdirectorout", n, "", 0))) {
      ...
      if (mono_aot_compatibility_flag) {
        Printf(
          tm2,
          "global::System.Runtime.InteropServices.GCHandle gcHandle = global::System.Runtime.InteropServices.GCHandle.FromIntPtr(swigDelegate%s_%s_Handle);\n",
          classname,
          methid);
        Printf(tm2,
               "    SwigDelegate%s_%s delegateSwigDelegate%s_%s = (SwigDelegate%s_%s) gcHandle.Target;\n",
               classname,
               methid,
               classname,
               methid,
    ...
    if (!ignored_method) {
      ...
      */
      ...
      Printf(
        director_callback_typedefs, "    typedef %s (SWIGSTDCALL* SWIG_Callback%s%s_t)(", c_ret_type, methid, mono_aot_compatibility_flag ? "_Dispatcher" : "");
      if (mono_aot_compatibility_flag) {
      ...
      if (!mono_aot_compatibility_flag) {
        ...
        Printf(director_mono_aot_delegate_definitions,
               " SwigDelegate%s_%s_Dispatcher(global::System.IntPtr swigDelegate%s_%s_Handle%s%s);\n",
               classname,
               methid,
               classname,
               methid,
