...
int OBJECTIVEC::functionWrapper(Node *n)
{
    ...
    if (proxy_flag && is_constructor)
    { // Handle constructor
        Setattr(n, "imfunctionname", wname);
        emitProxyClassConstructor(n);
    }
    else if (proxy_flag && is_destructor)
    { // Handle destructor
    ...
    if (proxy_flag && (global_variable_flag || global_func_flag) && !static_member_func_flag)
    { // Handle globals
        Setattr(n, "imfunctionname", wname);
        emitProxyGlobalFunctions(n);
    }
    else if (proxy_flag && (member_variable_flag || static_member_variable_flag || member_constant_flag || member_func_flag || static_member_func_flag))
    { // Handle members
        Setattr(n, "imfunctionname", wname);
...
void OBJECTIVEC::emitProxyClassConstructor(Node *n)
{
    ...
    for (p = parmlist; p; p = nextSibling(p), i++)
    {
        ...
        { // First valid argument, prefix it with 'With' and capitalize the first letter
...
{ // TODO: See this once more for nspaces.
    if (proxy_flag)
    {
