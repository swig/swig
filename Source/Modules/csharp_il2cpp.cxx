...
class CSHARP_IL2CPP:public Language {
...
public:
   ...
   * ----------------------------------------------------------------------------- */
    ...
    CSHARP_IL2CPP():empty_string(NewString("")),
  ...
  virtual void main(int argc, char *argv[]) {
    ...
    SWIG_config_file("csharp_il2cpp.swg");
  ...
  void emitProxyClassDefAndCPPCasts(Node *n) {
    ...
    if (feature_director) {
      ...
      // put this in classDirectorEnd ???
        String* csClassName = SwigType_namestr(proxy_class_name);
        Printf(proxy_class_code, "  // TODO: currently only support 1 instance in csharp side, maybe you should use a Dictionary<IntPr, YourObj> to store the mapping\n");
        Printf(proxy_class_code, "  private static %s SingleInstance = null;\n", csClassName);
        Printf(proxy_class_code, "  private static void RegistCSObject(%s csObj) {\n", csClassName);
        Printf(proxy_class_code, "    // Store mapping Key: cObjPtr(csObj.swigCPtr.Handle) to Value: csObj\n");
        Printf(proxy_class_code, "    SingleInstance = csObj;\n");
        Printf(proxy_class_code, "  }\n");
        Printf(proxy_class_code, "  private static %s GetPeerCSObject(global::System.IntPtr cObjPtr) {\n", csClassName);
        Printf(proxy_class_code, "    // Get csObj from mapping by Key cObjPtr\n");
        Printf(proxy_class_code, "    return SingleInstance;\n");
        Printf(proxy_class_code, "  }\n");
        Printf(proxy_class_code, "\n");
        Printf(proxy_class_code, "  private void SwigDirectorConnect() {\n");
        Printf(proxy_class_code, "    RegistCSObject(this);\n");
        Delete(csClassName);
      int i;
  ...
  int classDirectorMethod(Node *n, Node *parent, String *super) {
    ...
    String* DelegateType = NewStringf("SwigDelegate%s_%d", classname, n_dmethods-first_class_dmethod);
	Printf(callback_def, "  [AOT.MonoPInvokeCallback(typeof(%s))]\n  private static %s SwigDirectorMethod%s(", DelegateType, tm, overloaded_name);
    Delete(DelegateType);
    const String *csdirectordelegatemodifiers = Getattr(n, "feature:csdirectordelegatemodifiers");
	String *modifiers = (csdirectordelegatemodifiers ? NewStringf("%s%s", csdirectordelegatemodifiers, Len(csdirectordelegatemodifiers) > 0 ? " " : "") : NewStringf("[System.Runtime.InteropServices.UnmanagedFunctionPointer(System.Runtime.InteropServices.CallingConvention.StdCall)]\n  public "));
	Printf(director_delegate_definitions, "  %sdelegate %s", modifiers, tm);
	Delete(modifiers);
      } else {
	Swig_warning(WARN_CSHARP_TYPEMAP_CSTYPE_UNDEF, input_file, line_number, "No imtype typemap defined for %s\n", SwigType_str(returntype, 0));
      }
      Printf(delegate_parms, "global::System.IntPtr cObjPtr");
      //Printf(proxy_method_types, "global::System.IntPtr cObjPtr");
    }
    ...
    Printf(callback_code, "    %s csobj = GetPeerCSObject(cObjPtr);\n", SwigType_namestr(classname));
    String *upcall = NewStringf("csobj.%s(%s)", symname, imcall_args);
...
/* -----------------------------------------------------------------------------
 * swig_csharp_il2cpp()    - Instantiate module
 * ----------------------------------------------------------------------------- */

static Language *new_swig_csharp_il2cpp() {
  return new CSHARP_IL2CPP();
}
extern "C" Language *swig_csharp_il2cpp(void) {
  return new_swig_csharp_il2cpp();
}
...
const char *CSHARP_IL2CPP::usage = "\
C# Options (available with -cs_il2cpp)\n\
