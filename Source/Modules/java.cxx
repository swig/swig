...
public:
  ...
  void emitDirectorUpcalls() {
	...
	Putc('\n', dmethod_data);
      ...
      Printf(f_runtime, "    jmethodID director_methods[%d];\n", n_methods);
      Printf(f_runtime, "  }\n");
      Printf(f_runtime, "}\n");
      ...
      Printf(w->code, "  Swig::director_methods[i] = jenv->GetStaticMethodID(jcls, methods[i].method, methods[i].signature);\n");
      Printf(w->code, "  if (!Swig::director_methods[i]) return;\n");
      Printf(w->code, "}\n");
  ...
  int classDirectorMethod(Node *n, Node *parent, String *super) {
	...
	Printf(w->code, "jresult = (%s) ", c_ret_type);
      ...
      Printf(w->code, "jenv->%s(Swig::jclass_%s, Swig::director_methods[%s], %s);\n", methop, imclass_name, methid, jupcall_args);
