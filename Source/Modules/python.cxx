...
public:
  ...
  void appendOverloadDeclaration(File *dest, Node *n, const String *name, bool is_classmethod, bool is_staticmethod) {
    ...
    if (Len(parms) != 0 && Getattr(n, "feature:python:annotations:positional-only-overloads"))
      close = ", /)";
    Printv(dest, "def ", name, "(", parms, close, returnTypeAnnotationForStubFile(n, true), ":\n", tab, tab4, "...\n", NIL);
  }
   ...
   *
   * Write a .pyi stub declaration (header plus a '...' body) for a
   * non-member function into f_dest.
   * ------------------------------------------------------------ */
  ...
  */
  ...
  virtual int constructorHandler(Node *n) {
    ...
    if (shouldShowOverload(n) && add_init && (!builtin || pyi_stub)) {
      String *dest = pyi_stub ? f_stub : f_shadow;
      appendOverloadDeclaration(dest, n, "__init__", true, false);
    }
  ...
  void printClassHeader(Node *n, String *class_name, String *f_dest, bool addMetaclass) {
    ...
    String *metaclass = (addMetaclass && GetFlag(n, "feature:python:nondynamic")) ? NewString(", metaclass=_SwigNonDynamicMeta") : NewString("");
    Printv(f_dest, "class ", class_name, NIL);

    if (Len(base_class)) {
      Printf(f_dest, "(%s%s)", base_class, metaclass);
    } else {
      if (GetFlag(n, "feature:exceptionclass")) {
        Printf(f_dest, "(Exception%s)", metaclass);
      } else {
        Printf(f_dest, "(object%s)", metaclass);
      }
    }
    Delete(metaclass);
