...
va = python_pyiterator_rename_a.make_vector_a()
it = va.begin()
...
vb = python_pyiterator_rename_b.make_vector_b()
it = vb.begin()
if not hasattr(it, 'value'):
...
a_list = list(python_pyiterator_rename_a.make_vector_a())
if a_list != [10, 20, 30]:
    raise RuntimeError("module a: list() failed, got %s" % a_list)

b_list = list(python_pyiterator_rename_b.make_vector_b())
if b_list != [40, 50, 60]:
