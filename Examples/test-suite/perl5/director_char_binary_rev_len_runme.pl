...
BEGIN { use_ok 'director_char_binary_rev_len' }
require_ok 'director_char_binary_rev_len';

{
  package DirectorReverseLengthStringCallback;
  use base 'director_char_binary_rev_len::Callback';
  sub run {
      my $ret = 0;
...

{
  ...
  my $caller = director_char_binary_rev_len::Caller->new($callback);
  my $sum = $caller->call();
