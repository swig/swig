@file:JvmName("java_director_exception_feature_runme")
...
class MyFooDirectorImpl : Foo() {
    override fun ping(excp: Int): String {
        if (excp == 1) throw MyJavaException1(Consts.PINGEXCP1)
        if (excp == 2) throw MyJavaException2(Consts.PINGEXCP2)
        return "Ping director returned"
    }

    override fun pong(excp: Int): String {
        if (excp == 1) throw MyJavaException1(Consts.PONGEXCP1)
        if (excp == 2) throw MyJavaException2(Consts.PONGEXCP2)
        if (excp == 3) throw MyJavaUnexpected(Consts.PONGUNEXPECTED)
    ...
    override fun genericpong(excp: Int): String {
        if (excp == 1) throw MyJavaException1(Consts.GENERICPONGEXCP1)
...
fun main() {
    ...
    // The wrapped exceptions (MyJavaException1/2/Unexpected) override Throwable.getMessage()
