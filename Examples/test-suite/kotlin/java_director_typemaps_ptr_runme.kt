@file:JvmName("java_director_typemaps_ptr_runme")

// Test director pointer typemaps in typemaps.i - similar to java_director_typemaps.i testcase.
//
// This is the pointer (TYPE *OUTPUT) variant of java_director_typemaps (which uses the
...
// exercises only the non-NULL etest() path (identical to the java_director_typemaps reference
// variant). Supporting nulltest would require the Kotlin module to emit nullable array parameter
// types (JTYPE?) in the javadirectorin typemap for TYPE *OUTPUT.

import java_director_typemaps_ptr.*

class java_director_typemaps_ptr_MyQuux : Quux() {
        ...
        doublearg_inout: DoubleArray?)
    {
        ...
        if (signed_chararg_inout!![0].toInt() != 101) throw RuntimeException("unexpected value for signed_chararg_output")
        if (unsigned_chararg_inout!![0].toInt() != 101) throw RuntimeException("unexpected value for unsigned_chararg_output")
...
fun main() {
    ...
    val myquux = java_director_typemaps_ptr_MyQuux()
